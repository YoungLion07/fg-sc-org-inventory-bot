...
const data = new SlashCommandBuilder()
  .setName('set-handle')
  .setDescription('Save your own Star Citizen (RSI) handle to the org roster')
...
async function execute(interaction) {
  ...
  const { handle } = await members.setRsiHandle(interaction.user.id, interaction.options.getString('handle', true));
